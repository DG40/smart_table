...
class Board(object):
    ...
    def draw(self, strip, flag_set_up_brightness):
        # MY
        for i in range(8):
            for j in range(8):
                useful_box.battlefield[i][j] = 0
        ...
        for row_of_pieces in chunks(self.pieces, 8):
            ...
            for p in row_of_pieces:
                element = p.draw()
                useful_box.battlefield[p.x][p.y] = element
                row += element
            grid += '{0}{1}{0}\n'.format(str(i + 1), row)
            i += 1
        ...
        for i in range(8):
            print()
            for j in range(8):
                print(useful_box.battlefield[i][j], end='')
        print()
        
        # Print LEDs
        for i in range(8):
            for j in range(8):
                if useful_box.battlefield[i][j] == 'WW':  # my checkers
                    useful_box.color_cell(strip, i, j, (255, 0, 0, 0))
                elif useful_box.battlefield[i][j] == 'BB':  # computer checkers
                    useful_box.color_cell(strip, i, j, (0, 0, 255, 0))
                else:  # background
                    useful_box.color_cell(strip, i, j, (30, 0, 30, 0))
        strip.show()
        
        if flag_set_up_brightness:
